<template>
    <div>
        <p class="arrow-text" :class="roadmap.textColor">
            Step{{ getYear() }}
        </p>
        <canvas 
            v-resize="create"
...
export default {
    ...
    methods: {
        ...
        draw(canvasSize) {
            ...
            const bottomX = canvasSize * 0.35;
            this.text.titleX = canvasSize * 0.1;
            this.text.startX = canvasSize * 0.35;
        ...
        getCanvas() {
            const canvas = document.getElementById(this.getArrowId());
        ...
        drawText() {
            setTimeout(() => {
                ...
                for (let title of titles) {
                    const addY = y * row;
                    const addX = this.text.titleX * row;
                    this.ctx.fillText(title.trim(), this.text.titleX + addX, y + addY);
                    row++;
                }
                const addY = y * row;
        ...
        click() {
            this.$emit("clickArrow", this.index + 1);
        }
    }
}
</script>
